...
app.get('/',requireAuthentication,async c=>{
  ...
  if(s.activeCompanyId && !company){
    company=await c.env.DB.prepare(`SELECT id,company_identifier,display_name,legal_name,status,management_status FROM companies WHERE id=?`).bind(s.activeCompanyId).first<any>() as any;
  }
