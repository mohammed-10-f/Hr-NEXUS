  ...
  if (platform) {
    ...
    if (accessRaw) {
      ...
      const access = await c.env.DB.prepare(`
        SELECT cas.id,cas.company_id,cas.expires_at,cas.last_activity_at,c.company_identifier,c.display_name,c.legal_name,c.status
      ...
      `).bind(accessHash,platform.platform_user_id).first<{id:string;company_id:string;expires_at:string;company_identifier:string;display_name:string;legal_name:string;status:string}>();
      if (access) {
        if (Date.now()-new Date(access.last_activity_at).getTime()>INACTIVITY_TTL) { await c.env.DB.prepare(`UPDATE company_access_sessions SET revoked_at=CURRENT_TIMESTAMP WHERE id=?`).bind(access.id).run(); return {sessionId:platform.id,sessionType:'platform',platformUserId:platform.platform_user_id,companyUserId:null,activeCompanyId:null,accessMode:'platform',roles:['super_admin'],employeeId:null,mustChangePassword:false}; }
