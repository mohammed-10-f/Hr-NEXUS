...
export function AppShell(){
 const [open,setOpen]=useState(false),[expanded,setExpanded]=useState<string[]>(['الموظفون']),[ctx,setCtx]=useState<any>(null),[error,setError]=useState(false);
 ...
 <nav className="nav-list">{nav.map((item:any)=>item.children?<div className="nav-group" key={item.label}><button className="nav-parent" onClick={()=>toggle(item.label)}><item.icon size={19}/><span>{item.label}</span><ChevronLeft className={expanded.includes(item.label)?'rotated':''} size={15}/></button>{expanded.includes(item.label)&&<div className="nav-children">{item.children.map((child:any)=>child.to==='#'?<button key={child.label} className="nav-child disabled" disabled><span>{child.label}</span><em>لاحقاً</em></button>:<NavLink onClick={()=>setOpen(false)} key={child.label} className="nav-child" to={child.to}>{child.label}</NavLink>)}</div></div>:item.to==='#'?<button key={item.label} className="nav-parent disabled" disabled><item.icon size={19}/><span>{item.label}</span><em>لاحقاً</em></button>:<NavLink onClick={()=>setOpen(false)} key={item.label} className="nav-parent" to={item.to}><item.icon size={19}/><span>{item.label}</span></NavLink>)}</nav>
 ...
 {inCompany&&platform&&<button className="icon-btn" title="الخروج من الشركة" onClick={exitCompany}><LogOut size={18}/></button>}
 <button className="user-chip" onClick={logout}><div className="avatar">{platform?'م':'م'}</div><div><strong>{platform?'مدير المنصة':'مستخدم الشركة'}</strong><span>{platform&&inCompany?'Super Admin · Company Access':platform?'Platform':'Company User'}</span></div><ChevronLeft size={14}/></button></div></header>
