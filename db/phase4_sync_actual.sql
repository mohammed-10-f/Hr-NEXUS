-- HR Nexus Phase 4 — synchronization for the EXISTING hr-nexus D1 database.
-- Generated from the inspected current schema: organization_units already exists;
-- this script only adds the Phase 4 positions layer, indexes, integrity triggers,
-- and the missing organization permissions. It does not reset or delete data.
...
BEGIN
  SELECT CASE
    ...
    WHEN NOT EXISTS (SELECT 1 FROM organization_units WHERE id=NEW.parent_id AND company_id=NEW.company_id)
...
BEGIN
  SELECT CASE
    WHEN NOT EXISTS (SELECT 1 FROM organization_units WHERE id=NEW.organization_unit_id AND company_id=NEW.company_id)
      THEN RAISE(ABORT, 'ORG_POSITION_UNIT_COMPANY_MISMATCH')
    WHEN NEW.manager_position_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM positions WHERE id=NEW.manager_position_id AND company_id=NEW.company_id)
...
('organization.view','organization','view','عرض الهيكل التنظيمي','View organization','عرض الهيكل التنظيمي'),
...
WHERE r.code='company_admin' AND r.system_role=1 AND p.resource='organization';

-- Verification (read-only):
SELECT name, type, sql
FROM sqlite_master
WHERE name IN ('positions','idx_positions_company_unit','idx_positions_company_status','idx_positions_manager','idx_positions_company_code','trg_org_unit_parent_company_insert','trg_org_unit_parent_company_update','trg_position_company_refs_insert','trg_position_company_refs_update')
ORDER BY type,name;

SELECT id,resource,action,name_ar
FROM permissions
